-- Create ACS 5 yr 2024 place table with boundary features
CREATE OR REPLACE TABLE `teu_site_similarity.place_acs_5yr_2024_geo_features` AS 
SELECT 
  ...
  b.state_name,
  b.stusps,
  a.* EXCEPT(geo_id, year),
  b.geometry,
FROM`clgx-gis-app-dev-06e3.teu_site_similarity.acs_5yr_2024_sumlevel_place_features` a
LEFT JOIN `clgx-gis-app-dev-06e3.teu_site_similarity.boundaries_places_2024` b
ON a.geo_id = b.geoidfq;
...
-- For 32K places, this query took 31 MINUTES!   
CREATE OR REPLACE TABLE `clgx-gis-app-dev-06e3.teu_site_similarity.acs_5yr_place_parcel_features` AS
SELECT
  a.geoidfq,
...
-- This now includes land use code which is relevant to include condo properties
...
-- THIS TOOK 1 hour 50 minutes!! 
CREATE OR REPLACE TABLE `clgx-gis-app-dev-06e3.teu_site_similarity.acs_5yr_place_address_features` AS
SELECT
  a.geoidfq,
  COUNT(DISTINCT b.address_id) AS unq_addr_count,
  SUM(CASE WHEN b.land_use IN ('112','133') THEN 1 ELSE 0 END) AS condo_address_counts -- these land use codes have 3rd and 4th highest counts and represent condos
FROM `clgx-gis-app-dev-06e3.teu_site_similarity.place_acs_5yr_2024_geo_features` a
...
-- THIS TOOK 1 hour 22 minutes! Ouch
CREATE OR REPLACE TABLE `clgx-gis-app-dev-06e3.teu_site_similarity.acs_5yr_place_property_features` AS
SELECT
  a.geoidfq,
...
CREATE OR REPLACE TABLE `clgx-gis-app-dev-06e3.teu_site_similarity.acs_5yr_place_growth_v2_features` AS
...
-- Query to create a view for Firmographics
CREATE OR REPLACE TABLE `teu_site_similarity.stage_firmographics` 
CLUSTER BY fips AS
SELECT
  b.FIPS_STATE_CODE AS state_code,	
  b.FIPS_CODE AS fips,
  a.address_id,
  b.LOCATION_LATITUDE AS address_latitude,
  b.LOCATION_LONGITUDE AS address_longitude,
FROM `clgx-idap-bigquery-prd-a990.edr_ent_property_firmographics.vw_edr_firmographics_enterprise`a 
LEFT JOIN `clgx-idap-bigquery-prd-a990.edr_pmd_property_pipeline.vw_address_connect` b 
ON a.address_id = b.ADDRESS_ID;

-- Query to take counts of firmographics addresses
-- Query to create address features
-- THIS TOOK 1 hour 50 minutes!! 
CREATE OR REPLACE TABLE `clgx-gis-app-dev-06e3.teu_site_similarity.acs_5yr_place_firmographics_features` AS
SELECT
  a.geoidfq,
  COUNT(b.address_id) AS business_count
FROM `clgx-gis-app-dev-06e3.teu_site_similarity.place_acs_5yr_2024_geo_features` a
JOIN `clgx-gis-app-dev-06e3.teu_site_similarity.stage_firmographics` b
  ON a.state_fips = b.state_code
  AND ST_CONTAINS(a.geometry, ST_GEOGPOINT(b.address_longitude, b.address_latitude))
WHERE b.address_latitude IS NOT NULL
GROUP BY a.geoidfq;